...
for (const line of lines) {
    ...
    let copyRow = row.slice();
    for (let i = 1; i <= 5; i++) {
        const newElements = copyRow.map(item => (item + i) % 10);
        row.push(...newElements);
    }
    map.push(row);
}
// New rows to add to the map
let newRows = [];
for (let i = 1; i <= 5; i++) {
    for (const row of map) {
        const newRow = row.map(item => (item + i) % 10);
        newRows.push(newRow);
    }
}
map.push(...newRows);
...
const mapLength = map.flat().length;
// A*
while (openNodes.length > 0 && endNode == null) {
    ...
    if (closedNodes.length % 10000 === 0) {
        console.log(mapLength + ', ' + openNodes.length + ', ' + closedNodes.length);
    }
    if (bestNode.isGoal()) {
        endNode = bestNode;
    } else {
