...
let loser: Card | null = null;
let i = 0
for (i = 0; i < numbersDrawn.length && cards.length > 0; i++) {
    const winnersIndexes: number[] = [];
    for (let j = 0; j < cards.length; j++) {
        const card = cards[j];
        card.drawNumber(+numbersDrawn[i]);
        if (card.checkWinningState()) {
            winnersIndexes.push(j);
        }
    }
    // We're going to remove elements by their index, so we need to start removing them from the end
    winnersIndexes.reverse();

    for (const index of winnersIndexes) {
        cards.splice(index, 1);
    }
    
    if(cards.length === 1) {
        // The only card that hasn't won yet is the one that we'd like to have
        loser = cards[0];
    }
}

console.log('Score', loser?.getSumOfUnmarked()! * +numbersDrawn[i-1]);
